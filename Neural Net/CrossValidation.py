...
def torch_train(train_index, test_index, cross_data, Net, label):
    device = torch.device("cpu")

    pytorch_y = [[label] for label in label[train_index].to_numpy()]

    train_x = torch.Tensor(cross_data[train_index]).float().to(device)
    test_x = torch.Tensor(cross_data[test_index]).float().to(device)
    train_y = torch.Tensor(pytorch_y).float().to(device)

    pytorch_net = Net().to(device)
    ...
    for epoch in range(0, no_epochs):
        ...
        predictions = pytorch_net.forward(train_x)
...
def he_train(train_index, cross_data, label):
    cross_X_train = toNp(cross_data[train_index])
    cross_y_train = toNp(label[train_index].to_numpy())
    ...
    net.fit(cross_X_train, cross_y_train, epochs=400, learning_rate=0.001)
...
def cross_validate_he_torch(he_net, torch, heart_dataframe, label, he_training=True):
    ...
    for train_index, test_index in kf.split(cross_data):
        ...
        if he_training:
            cross_he_net = he_train(train_index, cross_data, label)
            he_out = cross_he_net.predict(cross_X_test)
            he_acc_sum.append(getAcc(he_out, cross_y_test))

        torch_net, torch_X_test = torch_train(train_index, test_index, cross_data, torch, label)
